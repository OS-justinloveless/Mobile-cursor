...
// Initialize workspace with dataDir from app locals
gitRoutes.use((req, res, next) => {
  if (!workspace && req.app.locals.dataDir) {
    workspace = new ProjectManager({ dataDir: req.app.locals.dataDir });
  }
  next();
});
...
gitRoutes.post("/:projectId/branch", async (req, res) => {
  try {
    ...
    let result;
    if (startPoint) {
      result = await gitManager.createBranchFrom(
        repoFullPath,
        name,
        startPoint,
        checkout !== false,
      ...
      result = await gitManager.createBranch(
        repoFullPath,
        name,
        checkout !== false,
      );
...
gitRoutes.get("/:projectId/diff", async (req, res) => {
  try {
    ...
    let result;
    if (commitHash) {
      result = await gitManager.getCommitDiff(
        repoFullPath,
        commitHash,
        file,
        limit,
      ...
      result = await gitManager.getDiff(
        repoFullPath,
        file,
        staged === "true",
        limit,
...
gitRoutes.get("/:projectId/log", async (req, res) => {
  try {
    ...
    const commits = await gitManager.getLog(
      repoFullPath,
      parsedLimit,
      parsedSkip,
    );
...
gitRoutes.get("/:projectId/scan-repos", async (req, res) => {
  try {
    ...
    const repositories = await gitManager.scanForRepositories(
      projectPath,
      options,
    );
...
gitRoutes.post("/:projectId/tag", async (req, res) => {
  try {
    ...
    const repoFullPath = await resolveRepoPath(projectId, repoPath);
    const result = await gitManager.createTag(
      repoFullPath,
      name,
      hash,
      message,
...
gitRoutes.post("/:projectId/reset", async (req, res) => {
  try {
    ...
    const repoFullPath = await resolveRepoPath(projectId, repoPath);
    const result = await gitManager.resetToCommit(
      repoFullPath,
      hash,
      mode || "mixed",
    );
...
gitRoutes.post("/:projectId/generate-commit-message", async (req, res) => {
  try {
    ...
    console.log(
      "[git.js] Generating commit message for project:",
      projectId,
      "repoPath:",
      repoPath,
    );
    ...
    const { stdout: detailedDiff } = await gitManager.execGit(
      repoFullPath,
      [
        "diff",
        "--cached",
...
Respond with ONLY the commit message, nothing else. No quotes, no explanation, just the message.`;
    ...
    console.log(
      "[git.js] Looking for available AI CLI tool to generate commit message...",
    );
    ...
    if (!availableTool) {
      return res.status(500).json({
        ...
        message:
          "Please install an AI CLI tool (claude, cursor-agent, or gemini)",
      });
    }

    console.log(
      `[git.js] Using ${availableTool.name} to generate commit message...`,
    );
    ...
    const result = await new Promise((resolve, reject) => {
      ...
      agent.on("close", (code) => {
        if (code !== 0) {
          console.error(
            `[git.js] ${availableTool.name} failed with code:`,
            code,
          );
          console.error("[git.js] stderr:", errorOutput);
          reject(
            new Error(
              `${availableTool.name} failed: ${errorOutput || "Unknown error"}`,
            ),
          );
        } else {
          resolve(output.trim());
