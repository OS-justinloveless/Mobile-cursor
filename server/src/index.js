...
function getClientDistPath() {
  ...
  console.warn(
    'Warning: Client dist directory not found. Run "npm run build" in client/ first.',
  );
  return devPath;
}
...
logger.info("Server", "Auth manager initialized", {
...
// Make DATA_DIR available to routes via app.locals
...
async function displayStartupMessage() {
  ...
  try {
    ...
    console.log(
      "║   (QR code generation failed - use manual connection below)      ║",
    );
  }
...
server.listen(PORT, "0.0.0.0", async () => {
  ...
  await chatProcessManager.loadPersistedConversations().catch((err) => {
    logger.error("Server", "Failed to load persisted conversations", {
      error: err.message,
    });
  });
